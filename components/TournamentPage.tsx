            ...
            <div className="border-b border-gray-700 mb-6">
                <nav className="flex space-x-4 overflow-x-auto">
                    {(['fixtures', 'table', 'leaders', 'teams'] as Tab[]).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`capitalize py-2 px-4 font-semibold rounded-t-lg ${activeTab === tab ? 'bg-gray-700 text-green-400' : 'text-gray-400 hover:bg-gray-700/50'} whitespace-nowrap`}>
                            {tab}
                        </button>
                    ))}
                </nav>
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between mt-2 gap-2 sm:gap-0">
                                <Link to={`/team/${teamA._id}`} className="flex items-center gap-3 w-full sm:w-2/5 hover:opacity-80 min-w-0">
                                    {teamA.logoUrl ? (
                                ...
                                <Link to={`/team/${teamB._id}`} className="flex items-center gap-3 w-full sm:w-2/5 justify-end hover:opacity-80 min-w-0">
                    ...
                    {tableData.map((t, i) => (
                        <tr key={t._id} className="border-b border-gray-700">
                            <td className="p-2 sm:p-3">
                                <Link to={`/team/${t._id}`} className="flex items-center gap-2 font-bold hover:opacity-80 min-w-0">
                                    <span className="w-6 flex-shrink-0">{i+1}</span>
                                    {t.logoUrl ? (
                                        <img src={t.logoUrl} alt={t.name} className="w-6 h-6 rounded-full object-cover flex-shrink-0"/>
                                    ) : (
                ...
                <ul className="space-y-2">
                    {data.map((item, index) => (
                        <li key={item.player._id} className="flex items-center justify-between text-sm p-2 rounded-md bg-gray-800/50">
                            <Link to={`/player/${item.player._id}`} className="flex items-center gap-3 hover:underline min-w-0">
                                <span className="font-bold w-6 text-center flex-shrink-0">{index + 1}</span>
                                {item.player.profile.imageUrl ? (
                                    <img src={item.player.profile.imageUrl} alt={item.player.profile.name} className="w-8 h-8 rounded-full object-cover flex-shrink-0"/>
                                ) : (
                                     <div className="w-8 h-8 rounded-full bg-gray-600 flex items-center justify-center flex-shrink-0">
                                        <span className="text-sm font-bold text-gray-400">{item.player.profile.name.charAt(0)}</span>
                                    </div>
                                )}
                                <span className="truncate">{item.player.profile.name}</span>
                            </Link>
                            <span className="font-extrabold text-lg text-green-400 flex-shrink-0">{item[metric]}</span>
                        </li>
                    ))}
